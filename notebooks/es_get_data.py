...
THORSTEN_NEUTRAL = {
    "22_10": {
        "url": "https://www.kaggle.com/datasets/bryanpark/spanish-single-speaker-speech-dataset/download?datasetVersionNumber=1",
        "dir_name": "spanish-single-speaker-speech-dataset",
...
def __text_normalization(json_file, num_workers=-1):
    ...
    text_normalizer = Normalizer(
        lang="es", input_case="cased", overwrite_cache=True, cache_dir=str(json_file.parent / "cache_dir"),
    )
    ...
    for metadata_fname in metadata:
        ...
        with open(meta_file, 'r') as fmeta:
            for line in tqdm(fmeta):
                ...
                # wav_file = unzipped_dataset_path / "19demarzo" / f"{wav_file_stem}.wav"
                # wav_file = unzipped_dataset_path / "asignaturaBueno" / f"{wav_file_stem}.wav"
                wav_file = unzipped_dataset_path / "esmapa152" / f"{wav_file_stem}.wav"
...
def main():
    ...
    # unzipped_dataset_path = data_root / "datasetCastellanoReducido"
    # unzipped_dataset_path = data_root / "datasetAlfonsoGutierrezFinetuning"
    unzipped_dataset_path = data_root / "datasetEsMapa152Finetuning"

    # Check if the unzipped dataset directory exists
    if not unzipped_dataset_path.exists():
        raise FileNotFoundError(f"Dataset directory not found: {unzipped_dataset_path}")
    else:
    ...
    entries_train, entries_val, entries_test, not_found_wavs, wrong_duration_wavs = __process_data(
        ...
        # metadata=unzipped_dataset_path/"metadata_train.csv",

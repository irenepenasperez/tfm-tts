...
# #########################################################################################
# class SpanishPhonemesTokenizer(BaseTokenizer):
#     # fmt: off
#     PUNCT_LIST = (  # Derived from LJSpeech and "/" additionally
#         ',', '.', '!', '?', '-',
#         ':', ';', '/', '"', '(',
...
#         add_blank_at=None,
#         pad_with_space=False,
#         text_preprocessing_func=lambda text: spanish_text_preprocessing(text),
#     ):
#         """English phoneme-based tokenizer.
#         Args:
#             g2p: Grapheme to phoneme module.
...
#         text = self.text_preprocessing_func(text)
#         # g2p_text = self.g2p(text)  # TODO: handle infer
#         return self.encode_from_g2p( text)
#         # return self.encode_from_g2p(g2p_text, text)
...
#######################################AQUÍ TENGO SHAPE DE 94 AHORA#########################################################
import logging

class SpanishPhonemesTokenizer(BaseTokenizer):
    PUNCT_LIST = (',', '.', '¡', '¿', '!', '?', '-', ':', ';', '/', '"', '(', ')', '[', ']', '{', '}')
    VOWELS = ('A', 'E', 'I', 'O', 'U', 'Á', 'É', 'Í', 'Ó', 'Ú', 'Ü','a', 'e', 'i', 'o', 'u', 'á', 'é', 'í', 'ó', 'ú', 'ü', 'AA', 'AE', 'AH', 'AO', 'AW','AY', 'EH', 'ER', 'EY', 'IH','IY', 'OW', 'OY', 'UH', 'UW',)
    CONSONANTS = ('B', 'C', 'CH', 'D', 'F', 'G', 'H', 'J', 'K', 'L', 'LL', 'M', 'N', 'Ñ', 'P', 'Q', 'R', 'RR', 'S', 'T', 'V', 'W', 'X', 'Y', 'Z', 'b', 'c', 'ch', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'll', 'm', 'n', 'ñ', 'p', 'q', 'r', 'rr', 's', 't', 'v', 'w', 'x', 'y', 'z', 'ZH', 'TH', 'SH', 'NG', 'HH', 'JH')
    def __init__(
        self,
        punct=True,
        non_default_punct_list=None,
    ...
    def encode_text(self, text):
        ps, space, tokens = [], self.tokens[self.space], set(self.tokens)
        for c in text:
            if c == space and len(ps) > 0 and ps[-1] != space:
                ps.append(c)
            elif c.isalnum() or c == "'":
                if c in tokens:
                    ps.append(c)
                else:
                    logging.warning(f"Text: [{text}] contains unknown char/phoneme: [{c}]. Symbol will be skipped.")
            elif c in self.PUNCT_LIST and self.punct:
                ps.append(c)
            else:
                logging.warning(f"Text: [{text}] contains unknown char/phoneme: [{c}]. Symbol will be skipped.")

        if ps and ps[-1] == space:
            ps.pop()
    ...
    def decode(self, tokens):
        text = ''.join([self._id2token[t] for t in tokens])
        return text
